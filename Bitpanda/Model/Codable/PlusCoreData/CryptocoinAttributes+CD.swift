...
extension CryptocoinAttributes {
    func save(context: NSManagedObjectContext) throws -> CryptocoinAttributesCD {
        guard let entity = NSEntityDescription.entity(forEntityName: "CryptocoinAttributesCD", in: context) else {
            throw CoreDataError.createEntityFailed("CryptocoinAttributesCD")
        }
        let attributes = CryptocoinAttributesCD(entity: entity, insertInto: context)
        ...
        do {
            ...
            throw CoreDataError.saveFailed("CryptocoinAttributesCD")
        }
